...
#    plt <- plt + scale_y_continuous(label = unit_format(unit = "M", scale = 1e-6, sep = "", accuracy = 0.1))
  ...
  output$donutTotal <- renderPlot({
    ggplot(dataDonut, aes(x = 2, y=count, fill = category)) + 
      ...
      geom_shadowtext( x = 1, y = 0, label = paste0("Confirmed\n", sum(dataTotal$count)), color="white", bg.colour = "red") + 
      theme(legend.position = "none", panel.grid = element_blank(), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), panel.background = element_rect(fill = "#4e5d6c"), axis.ticks.length = unit(0, "pt"), plot.margin = margin(0, 0, 0, 0, "pt"))
  })
  ...
  output$donutTotalUSA <- renderPlot({
    ggplot(dataDonutUSA, aes(x = 2, y=count, fill = category)) + 
      ...
      coord_polar(theta = "y", start=0) + 
      geom_text(aes(y = lab.pos, label = category), color = "black") + 
      geom_shadowtext( x = 1, y = 0, label = paste0("Confirmed\n", totalUSA$Confirmed), color="white", bg.colour = "red") + 
      theme(legend.position = "none", panel.grid = element_blank(), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank(), panel.background = element_rect(fill = "#4e5d6c"), axis.ticks.length = unit(0, "pt"), plot.margin = margin(0, 0, 0, 0, "pt"))
  })
